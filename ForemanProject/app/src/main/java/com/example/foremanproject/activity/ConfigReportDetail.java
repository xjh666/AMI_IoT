...
public class ConfigReportDetail extends AppCompatActivity {
    ...
    private void getReport(JSONObject response) throws JSONException {
        ...
        text.setText(" Reported at " + response.getString("reported_at").substring(0,10) + " " + response.getString("reported_at").substring(11,19) + " " +response.getString("reported_at").substring(23));
        list.addView(text);

        text = new TextView(this);
        text.setText("");
        text.setLayoutParams(new LinearLayout.LayoutParams(LinearLayout.LayoutParams.MATCH_PARENT, (int)(Configuration.getHeight()* 0.01)));
        ...
        text = new TextView(this);
        text.setText(" Report Time");
        list.addView(text); // space

        Iterator<?> keys = response.getJSONObject("metrics").getJSONObject("time").keys();
        while(keys.hasNext()) {
            String key = (String)keys.next();
            if(key.equals("total")) continue;

            if(!(response.getJSONObject("metrics").getJSONObject("time").getDouble(key)<0.001)){
                layout = new LinearLayout(this);
                ...
                text = new TextView(this);
                text.setText(" " + (response.getJSONObject("metrics").getJSONObject("time").getString(key).substring(0,6)));
                text.setTextSize(16);
                text.setTextAlignment(View.TEXT_ALIGNMENT_CENTER);
                text.setTextColor(Color.BLACK);
                text.setLayoutParams(new LinearLayout.LayoutParams((int)(Configuration.getWidth()* 0.505), LinearLayout.LayoutParams.MATCH_PARENT));
                text.setBackgroundResource(R.drawable.cell_shape);
                layout.addView(text);
        ...
        text = new TextView(this);
        text.setText(" " + (response.getJSONObject("metrics").getJSONObject("time").getString("total").substring(0,6)));
        text.setTextSize(16);
        text.setTextAlignment(View.TEXT_ALIGNMENT_CENTER);
        text.setTextColor(Color.BLACK);
        text.setLayoutParams(new LinearLayout.LayoutParams((int)(Configuration.getWidth()* 0.505), LinearLayout.LayoutParams.MATCH_PARENT));
